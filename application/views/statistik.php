...
$(function() {
	getStatistikKiriman();
<?php if ($role_id == 2) {
	echo "getStatistikBidding();";
}	?>
	
});

function getStatistikKiriman(result) {
	ajaxCall("statistik/getStatistikKiriman", null, function(result) {
...
<?php
	if ($role_id == 2) { ?>
	function getStatistikBidding() {
		ajaxCall('statistik/getStatistikBidding', null, function(result) {
			result = jQuery.parseJSON(result);
