...
class Penawaran_pro_model extends CI_Model
{
	...
	public function getOpenKiriman($user_id) {
		$query =  $this->db->query(
			...
			WHERE m.shipment_end_date > CURRENT_TIMESTAMP() AND m.shipment_status = -1 AND m.shipment_type = 2 AND t.shipment_id = m.shipment_id AND t.created_by = " . $user_id . "
	...
	public function getClosedKiriman($user_id) {
		$query =  $this->db->query(
			...
			FROM `m_shipment` m, (SELECT * FROM `t_bidding` WHERE created_by = " . $user_id . ") t
			WHERE m.shipment_status > -1 AND t.shipment_id = m.shipment_id AND m.shipment_type = 2
	...
	public function getKirimanCount($user_id) {
		$query = $this->db->query("
			...
			FROM `m_shipment` m, `t_bidding` t
			WHERE m.shipment_end_date > CURRENT_TIMESTAMP() AND m.shipment_status = -1 AND m.shipment_type = 2 AND t.shipment_id = m.shipment_id AND t.created_by = " . $user_id . "), 0) AS count_open,
